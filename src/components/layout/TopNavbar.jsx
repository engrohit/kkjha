...
const TopNavbar = () => {
    ...
    // Handle click outside for subscribe popup
    useEffect(() => {
    ...
    const socialLinks = [
        { icon: <FaLinkedinIn />, href: 'https://www.linkedin.com/in/krishan-kant-jha-a3b6a0163/', label: 'LinkedIn', color: 'hover:text-blue-700' },
        { icon: <FaInstagram />, href: 'https://www.instagram.com/artistkkjha/', label: 'Instagram', color: 'hover:text-pink-600' },
        { icon: <FaYoutube />, href: 'https://www.youtube.com/@artistkkjha', label: 'YouTube', color: 'hover:text-red-600' },
        { icon: <FaGlobe />, href: 'https://www.threads.com/@krishanjha93?xmt=AQF0e-MGlflipWPekNfHCXs36Rj_hx243bqyHrMf5jRYQMc', label: 'Threads', color: 'hover:text-black dark:hover:text-white' },
        { icon: <FaGlobe />, href: 'https://adbl.co/postcard', label: 'Audible', color: 'hover:text-orange-600' },
    ];
    ...
    return (
        ...
        >
            <div className={`mx-auto w-[98%] max-w-7xl rounded-2xl transition-all duration-300 ${scrolled
                ? 'bg-white/95 dark:bg-deep-navy/90 backdrop-blur-xl shadow-xl border border-slate-200/60 dark:border-white/15'
                : 'bg-white/80 dark:bg-deep-navy/60 backdrop-blur-lg border border-slate-200/40 dark:border-white/8 shadow-md'
                }`}
            >
                <div className="px-2 py-2 md:px-6 md:py-3 flex flex-nowrap justify-between items-center gap-2 relative">
                    ...
                    <div className="flex flex-wrap md:flex-nowrap items-center gap-1 md:gap-2 lg:gap-4">
                        ...
                        <div className={`relative ${isSearchOpen ? 'hidden md:block' : 'block'}`} ref={subscribeRef}>
                            <button
                                onClick={handleSubscribe}
                                className="flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-2 rounded-full bg-gradient-to-r from-cyber-blue to-neon-cyan text-white font-medium hover:shadow-lg hover:shadow-cyber-blue/30 transition-all shadow-md active:scale-95 group"
                            ...
                            <AnimatePresence>
                                {showSubscribeModal && (
                                    <motion.div
                                        ...
                                        className="absolute top-full right-0 mt-2 w-80 bg-white/95 dark:bg-deep-navy/95 backdrop-blur-xl rounded-xl p-5 shadow-2xl border border-slate-200 dark:border-white/10 z-[200]"
                                    >
                                        <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-2">Subscribe to Updates</h2>
                                        <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">Get notified about new stories, projects, and updates!</p>
                                        <form onSubmit={(e) => { e.preventDefault(); alert('Subscribed!'); setShowSubscribeModal(false); }}>
                                            <input
                                                type="email"
                                                placeholder="Enter your email"
                                                required
                                                className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-white/20 rounded-lg px-3 py-2 mb-3 text-sm focus:ring-2 focus:ring-green-500 outline-none text-slate-800 dark:text-white"
                                            />
                                            <div className="flex gap-2">
                                                <button
                                                    type="button"
                                                    onClick={() => setShowSubscribeModal(false)}
                                                    className="flex-1 px-3 py-1.5 rounded-lg border border-slate-300 dark:border-white/20 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors"
                                                >
                                                    Cancel
                                                </button>
                                                <button
                                                    type="submit"
                                                    className="flex-1 px-3 py-1.5 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-700 transition-colors"
                                                >
                                                    Subscribe
                                                </button>
                                            </div>
